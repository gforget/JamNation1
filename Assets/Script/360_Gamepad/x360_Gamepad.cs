...
public class x360_Gamepad
{
    ...
    public delegate void ConnectedStateChange(bool isConnected);
    public event ConnectedStateChange OnConnectedStateChange;
    ...
    public void Update()
    {
        ...
        if (connectedState != prevConnectedState && OnConnectedStateChange != null) OnConnectedStateChange(state.IsConnected);

        if (state.IsConnected)
            prevConnectedState = 1;
        else
            prevConnectedState = 2;
    ...
    public bool GetStickTapUp_L()
    {
        return (prev_state.ThumbSticks.Left.Y == 0f && state.ThumbSticks.Left.Y > 0.02f) ? true : false;
    }

    public bool GetStickTapDown_L()
    {
        return (prev_state.ThumbSticks.Left.Y == 0f && state.ThumbSticks.Left.Y < -0.02f) ? true : false;
    }

    // Return axes of right thumbstick
    public bool GetStickTapUp_R()
    {
        return (prev_state.ThumbSticks.Right.Y == 0f && state.ThumbSticks.Right.Y > 0.02f) ? true : false;
    }

    public bool GetStickTapDown_R()
    {
        return (prev_state.ThumbSticks.Right.Y == 0f && state.ThumbSticks.Right.Y < -0.02f) ? true : false;
    }
